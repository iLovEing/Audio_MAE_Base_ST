...
from .model_utils import trunc_normal_, window_partition, window_reverse, drop_path_f
...
class PatchEmbedInverse(nn.Module):
    ...
    def __init__(self, patch_size=4, out_c=1, embed_dim=96, extra_ds_ratio=1, norm_layer=nn.Identity()):
        super().__init__()
        patch_size = (patch_size*extra_ds_ratio, patch_size)
        self.patch_size = patch_size
        self.out_chans = out_c
        self.embed_dim = embed_dim
        self.norm = norm_layer(embed_dim)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        # cyclic shift
        if (H > self.window_size or W > self.window_size) and self.shift_size > 0:
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size,
        ...
        assert downsample is None or upsample is None, f'both downsample and upsample is not None'
...
class PatchSeparate(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.increase = nn.Linear(dim, 2 * dim, bias=False)
    ...
    def forward(self, x, H, W):
        ...
        x0 = x[..., : out_c]
        x1 = x[..., out_c: out_c*2]
        x2 = x[..., out_c*2: out_c*3]
        x3 = x[..., out_c*3: out_c*4]
        a = torch.stack([x0, x2], dim=2).reshape(B, H, 2 * W, out_c)
        b = torch.stack([x1, x3], dim=2).reshape(B, H, 2 * W, out_c)
        x = torch.stack([a, b], dim=2).reshape(B, 4*L, out_c)
        return x
